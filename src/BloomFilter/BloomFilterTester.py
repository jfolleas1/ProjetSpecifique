...
class BloomFilterTester:
    ...
    def __init__(self, n, m, point_build=[], discretisator=None):
        ...
        if discretisator:
            if self.discretisator:
                if self.discretisator.method_type == Method.DIS_TO_CHECK:
                    for pt in point_build:
                        self.bloom_filter.add(self.discretisator.discretise_point_to_one(pt).to_string())
                else:
                    for pt in point_build:
                        for d_pt in self.discretisator.discretise_point(pt):
    ...
    def test_set_points(self, points):
        ...
        number_of_positif = 0
        if self.discretisator:
            if self.discretisator.method_type == Method.DIS_TO_INSERT:
                for pt in points:
                    if self.test_one_point(self.discretisator.discretise_point_to_one(pt)):
                        number_of_positif += 1
            else:
                for pt in points:
                    for pt_d in self.discretisator.discretise_point(pt):
                        if self.test_one_point(pt_d):
                            number_of_positif += 1
                            break
        else:
            for pt in points:
